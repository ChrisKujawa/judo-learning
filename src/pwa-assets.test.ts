...
describe('PWA assets', () => {
  ...
  it('caches the app shell and falls back to the cached index for offline navigation', () => {
    ...
    expect(serviceWorker).toContain('BASE_PATH_WITHOUT_TRAILING_SLASH');
    expect(serviceWorker).toContain('url.pathname === BASE_PATH_WITHOUT_TRAILING_SLASH');
    ...
    expect(serviceWorker).toContain('const cached = await cache.match(assetUrl)');
    expect(serviceWorker).toContain('if (cached) return');
